...
function customMenu(p_position,p_menu,p_object) {
      ...
      for (var i=0; i<p_menu.length; i++) (function(i){
        ...
        if (p_menu[i].submenu!=undefined) {
          ...
          v_li.onmouseenter = function() {
            ...
            let v_ul_v_diff = v_ul_rect.y + v_ul_rect.height - window.innerHeight;
            if (v_ul_v_diff > 0) {
              v_ul.classList.add('aimara_menu_bottom');
            }
            // let v_ul_new_rect = v_ul.getBoundingClientRect();
            // let v_ul_new_v_diff = v_ul_new_rect.y;
            // if (v_ul_new_v_diff < 0) {
            //   v_ul.style.height = v_ul.offsetHeight + v_ul_new_v_diff + 'px';
            //   v_ul.style['overflow-y'] = 'scroll';
            // }
          }
      ...
      if (v_div_v_diff > 0) {
        ...
        v_div.classList.add('aimara_menu_bottom');
      }
}
