...
def create_parent_tab(request):
    ...
    json_object = json.loads(request.POST.get('data', None))
    v_database_index = json_object['p_database_index']
    v_tab_sequence = json_object['p_tab_sequence']
    v_mode = json_object['p_mode']
    v_conn_id = v_session.v_databases[v_database_index]['database'].v_conn_id
    ...
    try:
        ...
        '''.format(v_session.v_user_id, v_mode, v_tab_sequence, v_conn_id)
        ...
        v_parentTabId = int(v_database.ExecuteScalar(v_sql))

        v_database.Close()

        v_return['v_data'] = v_parentTabId
...
def create_child_tab(request):
    ...
    v_tab_sequence = json_object['p_tab_sequence']
    v_mode = json_object['p_mode']
    v_content = json_object['p_content']
    v_parent_tab_id = json_object['p_parent_tab_id']
    v_params = json_object['p_params']
    ...
    try:
        ...
        '''.format(v_parent_tab_id, v_mode, v_content, v_tab_sequence, v_params)
        ...
        v_sql = '''
            ...
            from child_tabs
            where parent_id = {0};
        '''.format(v_parent_tab_id)

        v_child_tab_id = int(v_database.ExecuteScalar(v_sql))

        v_database.Close()

        v_return['v_data'] = v_child_tab_id
    except Database.Exception as exc:
        v_return['v_error'] = True
        v_return['v_data'] = str(exc)
        return JsonResponse(v_return)
...
def update_child_tab(request):
    ...
    json_object = json.loads(request.POST.get('data', None))
    v_tab_id = json_object['p_tab_id']
    v_content = json_object['p_content'].replace("'", "''")
