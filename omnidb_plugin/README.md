...
sudo dpkg -i omnidb-plugin_2.12.1-debian-amd64.deb

# For example, for CentOS-like 64 bits:
sudo rpm -ivU omnidb-plugin_2.12.1-centos-amd64.rpm
```
...
```bash
wget --no-check-certificate https://omnidb.org/dist/2.12.1/omnidb-plugin_2.12.1-freebsd.tar.gz
```
...
```bash
tar -xzvf omnidb-plugin_2.12.1-freebsd.tar.gz
cp omnidb-plugin_2.12.1-freebsd/omnidb_plugin_10.so /usr/local/lib/postgresql/omnidb_plugin.so
```
