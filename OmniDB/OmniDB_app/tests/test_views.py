...
class Monitoring(TestCase):
    ...
    def test_monitoring_nosession(self):
        c = Client()
        response = c.post('/monitoring/', follow=True)
        assert '/login/' == response.redirect_chain[0][0]
        assert 302 == response.redirect_chain[0][1]

    def test_get_nodes_nosession(self):
        c = Client()
        response = c.post('/get_nodes/')
        assert 200 == response.status_code
        data = json.loads(response.content.decode())
        assert -1 == data['v_error_id']

    def test_new_node_nosession(self):
        c = Client()
        response = c.post('/new_node/')
        assert 200 == response.status_code
        data = json.loads(response.content.decode())
        assert -1 == data['v_error_id']

    def test_remove_node_nosession(self):
        c = Client()
        response = c.post('/remove_node/')
        assert 200 == response.status_code
        data = json.loads(response.content.decode())
        assert -1 == data['v_error_id']

    def test_refresh_node_key_nosession(self):
        c = Client()
        response = c.post('/refresh_node_key/')
        assert 200 == response.status_code
        data = json.loads(response.content.decode())
        assert -1 == data['v_error_id']

    def test_save_nodes_nosession(self):
        c = Client()
        response = c.post('/save_nodes/')
        assert 200 == response.status_code
        data = json.loads(response.content.decode())
        assert -1 == data['v_error_id']

    def test_get_alerts_nosession(self):
        c = Client()
        response = c.post('/get_alerts/')
        assert 200 == response.status_code
        data = json.loads(response.content.decode())
        assert -1 == data['v_error_id']

    def test_new_alert_nosession(self):
        c = Client()
        response = c.post('/new_alert/')
        assert 200 == response.status_code
        data = json.loads(response.content.decode())
        assert -1 == data['v_error_id']

    def test_remove_alert_nosession(self):
        c = Client()
        response = c.post('/remove_alert/')
        assert 200 == response.status_code
        data = json.loads(response.content.decode())
        assert -1 == data['v_error_id']

    def test_save_alerts_nosession(self):
        c = Client()
        response = c.post('/save_alerts/')
        assert 200 == response.status_code
        data = json.loads(response.content.decode())
        assert -1 == data['v_error_id']

    def test_get_alert_data_list_nosession(self):
        c = Client()
        response = c.post('/get_alert_data_list/')
        assert 200 == response.status_code
        data = json.loads(response.content.decode())
        assert -1 == data['v_error_id']

    def test_view_alert_chart_nosession(self):
        c = Client()
        response = c.post('/view_alert_chart/')
        assert 200 == response.status_code
        data = json.loads(response.content.decode())
        assert -1 == data['v_error_id']
