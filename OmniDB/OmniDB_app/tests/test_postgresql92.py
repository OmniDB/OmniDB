...
SELECT c.customerid, c.firstname, c.lastname, sum(o.totalamount) AS totalamount FROM (customers c JOIN orders o ON ((o.customerid = c.customerid))) GROUP BY c.customerid, c.firstname, c.lastname''' in data['v_data']
        self.database.v_connection.Execute('drop view vw_omnidb_test')
    ...
    def test_get_rule_definition_postgresql_session(self):
        ...
        assert 200 == response.status_code
        data = json.loads(response.content.decode())
        assert 'CREATE OR REPLACE RULE ru_test AS ON DELETE TO categories DO INSTEAD NOTHING;' in data['v_data']
        self.database.v_connection.Execute('drop rule ru_test on public.categories')
