...
# Creating folder GTM and coordinator
...
# Initializing and starting GTM
su - postgres -c "/usr/local/pgsql/bin/initgtm -Z gtm -D /usr/local/pgsql/data_gtm"
su - postgres -c "/usr/local/pgsql/bin/gtm -D /usr/local/pgsql/data_gtm >logfile 2>&1 &"

# Initializing coordinator
su - postgres -c "/usr/local/pgsql/bin/initdb -D /usr/local/pgsql/data_coord1 --nodename coord1"

PG_CONF="/usr/local/pgsql/data_coord1/postgresql.conf"
PG_HBA="/usr/local/pgsql/data_coord1/pg_hba.conf"
PG_DIR="/usr/local/pgsql/data_coord1"
...
echo "host    all           postgres     192.168.56.107/32    trust" >> "$PG_HBA"

# Append to pg_hba.conf to add password auth:
echo "host    all           all          all                  md5" >> "$PG_HBA"

# Starting coordinator
su - postgres -c "/usr/local/pgsql/bin/postgres --coordinator -D /usr/local/pgsql/data_coord1 >logfile 2>&1 &"
sleep 30
