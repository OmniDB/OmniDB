...
function createOmnisUiAssistant({p_callback_end = false, p_omnis, p_steps = []}) {
  ...
  var v_omnisControl = {
    ...
    updateOmnisPosition : function() {
      try {
        ...
        ? v_control.stepList[v_control.stepSelected].target()
        : v_control.stepList[v_control.stepSelected].target;
        var v_target_position = v_control.getPosition(v_target);
        var v_omnis_div = p_omnis.div;
  ...
  v_omnisControl.divCardElement.setAttribute('style', 'position:fixed; width:240px; max-width: 90vw; z-index: ' + v_omnisControl.z_index + 3 + '; box-shadow: 1px 0px 3px rgba(0,0,0,0.15); transition: all 0.45s ease 0.1s;');
